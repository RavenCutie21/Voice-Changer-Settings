...
Finally the new (actually pretty old) method that I use, I use [Werman's RN Noise removal](https://github.com/werman/noise-suppression-for-voice), by injecting it into my microphone, with [Equalizer APO](https://sourceforge.net/projects/equalizerapo/). It sounds complicated but it doesnt even create another virtual input like all the others. It's noise suppression is equal to Nvidia Broadcasts, with LESS usage than steelseries sonar. You can watch a video for learning how to set this up or DM me if you really want to. - extremely weird edit, on client mode this method just randomly does not work, playing around with the RN Noise removal and setting to 1.00, aka kill all sound entirely proved it to be working and then setting it back to the value i had before magically fixed it, and persisted on a client restart. SO in conclusion voodoo things here, just use something else until further investigation is done.

...
Finally the new (actually pretty old) method that I use, I use [Werman's RN Noise removal](https://github.com/werman/noise-suppression-for-voice), by injecting it into my microphone, with [Equalizer APO](https://sourceforge.net/projects/equalizerapo/). It sounds complicated but it doesnt even create another virtual input like all the others. It's noise suppression is equal to Nvidia Broadcasts, with LESS usage than steelseries sonar. You can watch a video for learning how to set this up or DM me if you really want to. - Bug: After Testing, Using the WASAPI driver, the MME driver, and the Wdinows DirectSound, it just works, no issues what so ever, using Client mode or server WDM-KS, it does not. So in conclusion here, it is using either Kernel Sound aka KS, but that doesnt make sense because over half the time it works on client mode, but never on WDM KS hybrid. SO if using this method please use Server Mode and please try to use WASAPI as its the one you ideally want to use.

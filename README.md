...
RMVPE_ONNX seems to just be the best option for everything I have tried so far. Try it yourself. NVIDIA can ignore it, but it has enabled even lower chunk access so still, try it. CREPE_TINY is just your fallback now if both RMVPE versions are causing issues for almost all cases (untested on MAC, and CPU users I would honestly just use colab there is no reason not to anymore)
